...
## At each step, a Logistic Regression is trained, and the regularization parameter C (1/lambda) is optimized using the hyperopt package (bayesian optimizer).
...
def L2_regularized_logit_rfe(X_stand, y, C):

	## calculate MCC through LOO with a given value of C
    
    lr = LogisticRegression(C=C, penalty='l2', solver='lbfgs', max_iter=10e5, random_state=42)
    y_pred = []
    loo = LeaveOneOut()
...
def run_optimization(X, y, n_iter, results):
    ...
    best = fmin(fn = lambda C: L2_regularized_logit_rfe(X_stand, y, C),
                space = hp.uniform('C', 0.001, 100), 
                algo=atpe.suggest, 
                max_evals = n_iter,
                trials=trials)
    
    lr = LogisticRegression(C=best['C'], 
                            penalty='l2', 
                            solver='lbfgs', 
                            max_iter=10e5, 
                            random_state=42)
    ...
    feat_rank = create_feature_ranking(X.columns, lr.coef_[0])
...
def repeat(data, file, trial):
    ...
    output.to_csv('./results/%s_lr_optimization_%i_iter_trial_%i.csv' % (file[4:-13], n_iter, trial))
...
def run_rfe_with_bayes(file):
    ...
    Parallel(n_jobs=10)(delayed(repeat)(data, file ,trial) for trial in range(2)) ### n_jobs is the number of cores that you want to use for the parallelization
