...
public:
	...
	double Qnet_inward_out[3];
	double Qnet_out[3];
	double Qnet, Qnet_inward;
	...
	void MeasureAPD90_INa_Qnet(double t, double Istim, double BCL, double dt, double Vm, double INa, double CaT, double Q1, double Q2);
...
void APInfor::MeasureAPD90_INa_Qnet(double t, double Istim, double BCL, double dt, double Vm, double INa, double CaT, double Q1, double Q2) {
	...
	if (((Istim >= 1e-3 || Istim <= -1e-3 ) && (fabs(Istim_prev) < 1e-10))) {
		...
		CaD_peak_T_out[APD_out_swhich] = CaD_peak_T;
		dCadt_time_out[APD_out_swhich] = dCadt_time;
		Qnet_out[APD_out_swhich] = Qnet;
		Qnet_inward_out[APD_out_swhich] = Qnet_inward;
...
void APInfor::ReportLastThree() {
	...
	for (int i = 0; i < 3; ++i)
	{
		std::cout << std::setprecision(9) << current_stim_counter[i] << " "
		          ...
		          << CaD_peak_T_out[i]  << " "
		          << dCadt_time_out[i]  << " "
		          << Qnet_out[APD_out_swhich] << " "
		          << Qnet_inward_out[APD_out_swhich] << " ";
...
void APInfor::ReportLast100() {


	plateau_potential /= (50.0 - 10.0); // average

	APD_out_swhich =  Stim_counter % 1;
	APD_out_end[APD_out_swhich] = APD90;
	APD75_out[APD_out_swhich] = APD75;
	APD50_out[APD_out_swhich] = APD50;
	APD30_out[APD_out_swhich] = APD30;
